...
/**
 * 장소 댓글 이미지 업로드 presigned URL 발급. 이미지는 1장만 허용한다.
 * 발급 권한은 댓글 작성 권한과 동일하다(PlaceCommentService.checkMapMember와 동일 규칙).
 */
@Service
public class PlaceCommentPhotoService {

    private static final long MAX_FILE_SIZE = 5L * 1024 * 1024;
    private static final Set<String> ALLOWED_CONTENT_TYPES = Set.of("image/jpeg", "image/png", "image/webp");
    private static final long EXPIRES_IN_SECONDS = 300L;
    private static final String KEY_PREFIX = "place-comments/";

    private final PlaceRepository placeRepository;
    private final MapClient mapClient;
    private final ObjectStoragePresigner objectStoragePresigner;

    public PlaceCommentPhotoService(PlaceRepository placeRepository, MapClient mapClient,
    ...
    public CommentPhotoUploadUrlResponse issueUploadUrl(Long placeId, CommentPhotoUploadUrlRequest request, Long userId) {
        if (userId == null) {
        ...
        return new CommentPhotoUploadUrlResponse(
